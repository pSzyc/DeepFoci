...
def evaluate_detections_all(
    T, h, d, filenames_masks, filenames_results, evaluate_index
):

    dices = []
    for file_num, (filename_result, filename_mask) in enumerate(
        zip(filenames_results, filenames_masks)
    ):
...
class WrapperEvaluateDetections(object):
    ...
    def __call__(self, **params):
        return evaluate_detections_all(
            params["T"],
            params["h"],
            params["d"],
            self.filenames_masks,
            self.filenames_results,
            self.evaluate_index,
        )
